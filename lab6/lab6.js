...
function testTime() {
    ...
    time = setInterval(function () {
        let myDate = new Date();
        if (myDate.getSeconds() > 55 || count >= 10) {
            clearInterval(time);
            value *= 2;
            console.log(value);
            console.log("testTime结束");
        } else {
            value *= 2;
            count += 1;
            console.log(value);
...
function testMail(telephone,mail) {
    let TelPattern = /^1[3-9]\d{9}$/;
    let MailPattern = /^[A-Za-z0-9\u4e00-\u9fa5]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/;
    let x1 = TelPattern.test(telephone)? "right" : "wrong";
    let x2 = MailPattern.test(mail)? "right" : "wrong" ;
    console.log("The telephone is " + x1 +" and the mail is " + x2+"!");
}
...
function testKeyBoard(wantInput, actualInput) {
    ...
    for (let i = 0, j = 0; i < wantInput.length; i++) {
        if (wantInput.charAt(i) !== actualInput.charAt(i - j)) {
            out.add(wantInput.charAt(i).toUpperCase());
            j++;
        }
    }
    console.log(out);
...
testTime();
function test(){
   console.log(testMail(13681977548,"17307100018@fudan.edu.cn"));
